// Enhanced bl-dialog.component.ts - Updated for component-specific bls
import { Component, Input, Output, EventEmitter, OnInit, OnDestroy } from '@angular/core';
import { StudentWithPayment } from '../../../models/payment.model';

@Component({
  selector: 'app-bl-dialog',
  template: `
    <div class="bl-dialog-overlay bl-print-container" (click)="onOverlayClick($event)">
      <div class="bl-dialog-container" (click)="$event.stopPropagation()">
        <div class="bl-dialog-header no-print">
          <div class="header-content">
            <div class="header-icon">📋</div>
            <div class="header-text">
              <h2>
                bon de livraison - {{ student?.name }}
...
export class BlDialogComponent implements OnInit, OnDestroy {
...
@Input() componentOnly?: 'uniform' | 'inscriptionFee' | 'tuition'; // ✅ ADD 'tuition'
  ...
  ngOnInit(): void {
    ...
    document.body.classList.add('bl-dialog-open');
  ...
  ngOnDestroy(): void {
    ...
    document.body.classList.remove('bl-dialog-open');
  ...
  closeDialog(): void {
    const container = document.querySelector('.bl-dialog-container') as HTMLElement;
    const overlay = document.querySelector('.bl-dialog-overlay') as HTMLElement;
...
getblTypeDescription(): string {
  ...
  // ✅ ADD: Handle tuition-only monthly bls
  if (this.componentOnly === 'tuition' && this.showCurrentMonthOnly && this.monthName) {
    return `bon de livraison frais scolaires - ${this.monthName}`;
  }
  if (this.showCurrentMonthOnly && this.monthName) {
    return `bon de livraison mensuelle - ${this.monthName}`;
  }
  return 'bon de livraison cumulative';
}
}