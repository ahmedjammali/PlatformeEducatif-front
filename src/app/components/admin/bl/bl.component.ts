// bl.component.ts - Modified with PDF fixes and component-specific bls
...
export class blComponent implements OnInit {
  ...
  // NEW: Component-specific bl inputs
  // NEW: Component-specific bl inputs
@Input() componentOnly?: 'uniform' | 'inscriptionFee' | 'tuition'; // ✅ ADD 'tuition'
  ...
  private generateblData(): void {
    ...
    const blNumber = `INV-${this.academicYear}-${studentInitials}-${timestamp.toString().slice(-6)}`;
    ...
    const currentMonthInfo = this.getCurrentMonthInfo();
     const blDate = this.getblDate();
    this.blData = {
      student: this.student,
      academicYear: this.academicYear,
         generatedDate: blDate, // ✅ Changed from new Date()
      blNumber,
      payments,
      currentMonthAmounts,
      remainingAmounts,
  ...
  private getblDate(): Date {
  // For component-specific bls, use the component's payment date
  ...
  // For monthly bls, use the specific month's payment date
  ...
  // For cumulative bls, use the most recent payment date
  ...
  // Handle tuition-only bls
  if (this.componentOnly === 'tuition') {
    let tuitionAmount = 0;
    
    if (this.showCurrentMonthOnly && this.currentMonthIndex !== undefined) {
      // For monthly tuition bls, get the full monthly amount (not just paid amount)
      const monthPayment = this.student.paymentRecord?.tuitionMonthlyPayments?.[this.currentMonthIndex];
      tuitionAmount = monthPayment?.amount || 0; // Full monthly amount
    } else {
      // For cumulative tuition bls, get total paid tuition
  ...
  getTVAAmount(component: 'tuition' | 'uniform' | 'transportation' | 'inscriptionFee'): number {
    switch (component) {
      case 'tuition':
        return this.blData?.tva?.tuitionTVA || 0;
      case 'uniform':
        return this.blData?.tva?.uniformTVA || 0;
      case 'transportation':
        return this.blData?.tva?.transportationTVA || 0;
      case 'inscriptionFee':
        return this.blData?.tva?.inscriptionFeeTVA || 0;
      default:
        return 0;
    }
  }
...
getHTAmount(component: 'tuition' | 'uniform' | 'transportation' | 'inscriptionFee'): number {
  if (!this.blData) return 0;
  
  switch (component) {
    case 'tuition':
      // ✅ CHANGE: For monthly bls, show the full monthly amount
      if (this.showCurrentMonthOnly && this.currentMonthIndex !== undefined) {
        const monthPayment = this.student.paymentRecord?.tuitionMonthlyPayments?.[this.currentMonthIndex];
        return monthPayment?.amount || 0; // Show full amount, not just paid amount
      }
      return this.blData.currentMonthAmounts.tuition;
    case 'uniform':
      return this.blData.currentMonthAmounts.uniform;
    case 'transportation':
      // ✅ CHANGE: For monthly bls, show the full monthly amount
      if (this.showCurrentMonthOnly && this.currentMonthIndex !== undefined) {
        const transportPayments = this.student.paymentRecord?.transportation?.monthlyPayments || [];
      ...
      return this.blData.currentMonthAmounts.transportation;
    case 'inscriptionFee':
      return this.blData.currentMonthAmounts.inscriptionFee;
    default:
      return 0;
  }
}
...
getOriginalAmountForCurrentMonth(component: 'tuition' | 'uniform' | 'transportation' | 'inscriptionFee'): number {
  ...
  switch (component) {
    ...
    case 'uniform':
      return this.blData.currentMonthAmounts.uniform > 0 ? 
             (this.student.paymentRecord?.uniform?.price || 0) : 0;
    case 'transportation':
      const transportPayments = this.student.paymentRecord?.transportation?.monthlyPayments || [];
      const monthTransport = transportPayments[this.currentMonthIndex];
    ...
    case 'inscriptionFee':
      return this.blData.currentMonthAmounts.inscriptionFee > 0 ? 
             (this.student.paymentRecord?.inscriptionFee?.price || 0) : 0;
    default:
      return 0;
  }
...
private getUsedRowsCount(): number {
    ...
    if (this.componentOnly === 'uniform') {
      return this.hasUniformForCurrentbl() ? 1 : 0;
    }
    
    if (this.componentOnly === 'inscriptionFee') {
      return this.hasInscriptionFeeForCurrentbl() ? 1 : 0;
    }
    
    // ✅ ADD: Handle tuition-only bls
    if (this.componentOnly === 'tuition') {
      return this.getHTAmount('tuition') > 0 ? 1 : 0;
    }
    
    if (this.getHTAmount('tuition') > 0) count++;
    if (this.hasUniformForCurrentbl()) count++;
    if (this.hasTransportationForCurrentbl()) count++;
    if (this.hasInscriptionFeeForCurrentbl()) count++;
    
    return count;
  }

hasUniformForCurrentbl(): boolean {
  if (this.componentOnly === 'tuition') return false; // ✅ ADD: Block uniform for tuition-only bls
  ...
  if (this.showCurrentMonthOnly) {
    return (this.blData?.currentMonthAmounts?.uniform || 0) > 0;
  }
  
  return true;
}

hasTransportationForCurrentbl(): boolean {
  if (this.componentOnly === 'tuition') return false; // ✅ ADD: Block transportation for tuition-only bls
  if (this.componentOnly) return false;
  if (!this.hasTransportation()) return false;
  
  if (this.showCurrentMonthOnly) {
    return (this.blData?.currentMonthAmounts?.transportation || 0) > 0;
  }
...
hasInscriptionFeeForCurrentbl(): boolean {
  if (this.componentOnly === 'tuition') return false; // ✅ ADD: Block inscription fee for tuition-only bls
  ...
  if (this.showCurrentMonthOnly) {
    return (this.blData?.currentMonthAmounts?.inscriptionFee || 0) > 0;
  }
  ...
  async downloadPDF(): Promise<void> {
    ...
    try {
      const blElement = document.querySelector('.bl-content') as HTMLElement;
      if (!blElement) {
        console.error('bl content not found');
      ...
      const clonedbl = blElement.cloneNode(true) as HTMLElement;
      this.optimizeForPDF(clonedbl);
      pdfContainer.appendChild(clonedbl);
      ...
      // Generate filename
      let fileName = `Bon de livraison_${this.student.name.replace(/\s+/g, '_')}_${this.blData.blNumber.split('-').pop()}`;
      
      if (this.componentOnly === 'uniform') {
        fileName += '_Uniforme';
      } else if (this.componentOnly === 'inscriptionFee') {
        fileName += '_FraisInscription';
      } else if (this.showCurrentMonthOnly && this.blData.currentMonthInfo) {
        fileName += `_${this.blData.currentMonthInfo.monthName}`;
      }
  ...
  hasDiscount(): boolean {
    return this.blData?.discount?.enabled || false;
  }

  getOriginalTuitionAmount(): number {
    return this.blData?.discount?.originalTuitionAmount || 0;
  }

  getDiscountAmount(): number {
    return this.blData?.discount?.discountAmount || 0;
  }

  getDiscountPercentage(): number {
    return this.blData?.discount?.percentage || 0;
  }

getblTitle(): string {
  if (this.componentOnly === 'uniform') {
    return 'Bon de livraison - UNIFORME SCOLAIRE';
  }
  if (this.componentOnly === 'inscriptionFee') {
    return 'Bon de livraison - FRAIS D\'INSCRIPTION';
  }
  // Add this new condition:
  if (this.componentOnly === 'tuition' && this.showCurrentMonthOnly && this.blData?.currentMonthInfo) {
    return `Bon de livraison - FRAIS SCOLAIRES ${this.blData.currentMonthInfo.monthName.toUpperCase()}`;
  }
  if (this.showCurrentMonthOnly && this.blData?.currentMonthInfo) {
    return `Bon de livraison - ${this.blData.currentMonthInfo.monthName.toUpperCase()}`;
  }
  ...
  getTuitionPeriodDescription(): string {
    if (this.showCurrentMonthOnly && this.blData?.currentMonthInfo) {
      return `FRAIS SCOLAIRES - ${this.blData.currentMonthInfo.monthName.toUpperCase()} ${this.academicYear}`;
    }
  ...
  getTransportationPeriodDescription(): string {
    if (this.showCurrentMonthOnly && this.blData?.currentMonthInfo) {
      return `TRANSPORT - ${this.getTransportationType().toUpperCase()} - ${this.blData.currentMonthInfo.monthName.toUpperCase()}`;
    }
    return `TRANSPORT - ${this.getTransportationType().toUpperCase()}`;
  }

  // Ajouter cette méthode dans votre composant bl.component.ts

private referenceCounter = 1;

getNextReferenceNumber(): string {
  // Reset counter for each bl generation
  if (this.referenceCounter === 1) {
    ...
    // Check uniform
    if (this.hasUniformForCurrentbl() && this.getHTAmount('uniform') > 0) {
      counter++;
    }
...
getblItemNumber(): number {
  ...
  if (!this.componentOnly && this.hasTransportationForCurrentbl() && this.getHTAmount('transportation') > 0) {
    itemCount++;
  }
