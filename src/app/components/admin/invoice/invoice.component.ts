    ...
    @Input() componentOnly?: 'uniform' | 'inscriptionFee' | 'tuition'; // ✅ ADD 'tuition'
      ...
      // Replace your calculateCurrentMonthAmounts() method in invoice.component.ts with this:
  ...
  private calculateCurrentMonthAmounts(): any {
    ...
    if (this.componentOnly === 'tuition') {
      ...
      if (this.showCurrentMonthOnly && this.currentMonthIndex !== undefined) {
        // For monthly tuition invoices, get the full monthly amount
        const monthPayment = this.student.paymentRecord?.tuitionMonthlyPayments?.[this.currentMonthIndex];
        tuitionAmount = monthPayment?.amount || 0;
      } else {
    ...
    // ✅ CUMULATIVE INVOICE (no month filter)
    ...
    // ✅ MONTHLY INVOICE - Calculate amounts for specific month only
    let tuitionAmount = 0;
    let uniformAmount = 0;
    let transportationAmount = 0;
    let inscriptionFeeAmount = 0;

    // Get tuition for this specific month
    if (this.currentMonthIndex !== undefined && this.student.paymentRecord?.tuitionMonthlyPayments) {
      const monthPayment = this.student.paymentRecord.tuitionMonthlyPayments[this.currentMonthIndex];
      if (monthPayment) {
        // ✅ Use full monthly amount, not just paid amount
        tuitionAmount = monthPayment.amount || 0;
      }
    }

    // Check if uniform was paid this month
    ...
    // Check if inscription fee was paid this month
    ...
    // ✅ FIX: Get transportation for this specific month ONLY
    if (this.hasTransportation() && this.currentMonthIndex !== undefined) {
      ...
      if (monthTransportPayment) {
        // ✅ Use the month's amount, not paidAmount (which could be cumulative)
        transportationAmount = monthTransportPayment.amount || 0;
      }
    }
